...
int main()
{
	...
	lp(1, i, n + 1)
	{
		...
		if (a[i] == 2)
		{
			if (a[i - 1] == 3 && a[i - 2] == 1 && i - 2 > 0 && !z[i - 1] && !z[i - 2])
				ans++;
			else if (a[i - 1] == 2 && !z[i - 1] && i - 1 > 0)
				ans++;
		} else if (a[i] == 1)
		{
			if (a[i - 1] == 3 && a[i - 2] == 2 && i - 2 > 0 && !z[i - 1] && !z[i - 2])
				ans++;
			else if (a[i - 1] == 1 && !z[i - 1] && i - 1 > 0)
				ans++;
		} if (a[i] == 3 && a[i - 1] == 1 && !z[i - 1])
			a[i] = 2;
		else if (a[i] == 3 && a[i - 1] == 2 && !z[i - 1])
			a[i] = 1;
		if (ans > c)
			z[i] = true;
	}
